#!/usr/bin/env python3
"""Build the deterministic, self-contained QEVA v0.5 project ZIP."""
...
PREFIX = PurePosixPath("qeva-site-v0.5")
...
def verify() -> None:
    ...
    result = subprocess.run(
        [sys.executable, "scripts/verify.py", "--quiet"],
        cwd=PROJECT / "dist",
        env=environment,
        check=False,
    )
...
def build(output: Path) -> str:
    ...
    try:
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            archive.comment = b"QEVA 0.5.0 deterministic project release"
