...
export default function AlignPillDemo() {
  ...
  const [why, setWhy] = useState<{name:string; value:number}[]>([]);
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState<string>("");

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("qeva_goals") || "[]");
  ...
  async function onCompile(tpl:"slides"|"one_pager"|"flashcards") {
    ...
    try {
      ...
      a.download = `${data.artifact.id}.json`;
      a.click();
    } catch (e:any) {
      setErr(String(e?.message || e));
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-black/40 p-5">
      ...
      <textarea
        ...
        className="w-full h-40 rounded-xl bg-black/30 p-3"
        placeholder="Paste selection…"
      />
      <div className="mt-3 flex items-center gap-3">
        ...
        {!!why.length && <span className="text-xs text-slate-400">
          why: {why.map(w=>`${w.name}:${Number(w.value).toFixed(2)}`).join(", ")}
        </span>}
      </div>
